...
$(document).ready(function(){  
    ...
    $("#getServiceCallsList").click(function(){
		if(!connected){
			$('#notConnectedError').modal('show');
			return;
		}
...
function displayServiceCalls(json){
	var x = json.value;
...
//	alert(JSON.stringify(json));
	...
	for (var i=0;i<x.length;i++){
		$("#resultTable tbody").append(
				"<tr>"+
				"<td>"+json.value[i].DocNum+"</td>"+
				"<td>"+json.value[i].CustomerCode+"</td>"+
				"<td>"+json.value[i].CustomerName+"</td>"+
				"<td>"+json.value[i].CreationDate+"</td>"+
				"<td>"+json.value[i].Subject+"</td>"+
